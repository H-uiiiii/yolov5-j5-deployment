...
- 🔧 **全流程落地**：PyTorch → ONNX → INT8 量化 → BPU 编译 → C++ 推理 → CAN 输出
- 📊 **量化精度对齐**：余弦相似度 **0.999**（地平线工具链输出）
- 🚗 **车规级输出**：自定义 SocketCAN 双帧协议，candump 逐字节验证
- ⚡ **性能评测**：30 张图批量测试，**424 个目标零空检测**
- 🔬 **多框架对比**：ONNX Runtime / NCNN / MNN / LibTorch 四引擎 × CPU/Vulkan/OpenCL 多后端横向评测
- 📝 **踩坑记录**：18 个实际问题的定位与解决过程
